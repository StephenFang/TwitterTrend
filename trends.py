...
def analyze_tweet_sentiment(tweet):
    ...
    if len(new_list) == 0:
        ...
        for x in new_list:
            total = total + sentiment_value(get_word_sentiment(x))
        return make_sentiment(total/len(new_list))
...
def find_centroid(polygon):
    ...
    while i < (len(polygon) - 1):
        xi, xi_plus1, yi, yi_plus1 = latitude(polygon[i]), latitude(polygon[i+1]), longitude(polygon[i]), longitude(polygon[i+1])
        area = area + (xi * yi_plus1 - xi_plus1 * yi)
        lat_sum = lat_sum + (xi + xi_plus1) * (xi * yi_plus1 - xi_plus1 * yi)
        lon_sum = lon_sum + (yi + yi_plus1) * (xi * yi_plus1 - xi_plus1 * yi)
        i = i + 1
...
def group_tweets_by_state(tweets):
    ...
    for element in tweets: 
        state = closest_state(element)
        if state in tweets_by_state:
            tweets_by_state[state].append(element)
        else:
            tweets_by_state[state] = [element]
